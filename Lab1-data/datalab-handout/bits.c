 ...
 *   Rating: 1
 *   返回最小的32位整数
 *   思路：32位补码整数最小的位 1000,0000,0000,0000,0000,0000,0000,0000.
 */
int tmin(void) {
  return 1 << 31;
}
...
int isLessOrEqual(int x, int y) {
    ...
    int minx = !(x^(1 << 31));
    int result = !((y + (~x) + 1) >> 31);
  return (result|fe|minx|fxyz)&(fyzx);
}
//4
/* 
...
unsigned float_twice(unsigned uf) {
  ...
  if (!exp) return (uf << 1)| sig;
  if (!(exp ^ 255)) return uf;
  exp = exp + 1;
  if (!(exp^255)) return 0x7f800000 | sig;
